...
@observer
class AdLiveList extends React.Component {
    ...
    componentDidMount() {
    	...
    	this.props.adLiveStore.clearAdStore();
       
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
  
    	if(this.props.adLiveStore.isLoadFailed) {
            this.props.enqueueSnackbar("서버 목록 조회에 실패하였습니다", {
                variant: 'error'
            });

            this.props.adLiveStore.clearListState();
        }
    ...
    handleOpenDialog = () => {
        this.props.adLiveStore.openDialog();
    };
    
    handleChangeLocation = (e) => {
   	 	const name = e.target.name;
        let value = e.target.value;
        this.props.adLiveStore.changeNewShowCtg(name, value);
        
        this.props.adLiveStore.loadAds();
    ...
    //	alert('mov_id -> '+mov_id+' 상세정보 조회 api ');
    	...
    	this.props.adLiveStore.loadAdDetail();
    ...
    handleDelImageChnList  = (ad_img_idx) => {
    	...
    	this.props.adLiveStore.delImageChnList(ad_img_idx);
    }
    
    handleChangePagingPage = (event, page) => {
        this.props.adLiveStore.changePagingPage(page);
    };

    handleChangePagingRowsPerPage = (event) => {
        const newValue = event.target.value;

        this.props.adLiveStore.changePagingRowsPerPage(newValue);
    };
    ...
    handleChangeActor = (e) => {       
        ...
        let value = e.target.value;
      //  alert(name+":"+value);
       // this.props.adLiveStore.changeNewMovie(name, value);
    };
    
    handleChangeNewAd = (e) => {       
        const name = e.target.name;
        let value = e.target.value;
        this.props.adLiveStore.changeNewAd(name, value);
    };
    ...
    handleChangeUploadMovieFile = (event) => {
        ...
        if( event.target.files[0] ){
            const file = event.target.files[0];
           	this.props.adLiveStore.changeNewMovie(name, file);
        }
    };
    ...
    handleChangeUploadImageFile = (event, langCd) => {
        ...
        if( event.target.files[0] ) { 
        	const file = event.target.files[0];
        	this.props.adLiveStore.changeNewCover(name, file, langCd);        	
        }
    };
    ...
    handleChangeLinkAddr = (event, langCd) => {
        ...
        this.props.adLiveStore.changeLinkAddr(name, value, langCd);
    };
    
    handleAddImageChnList = (event) => {
    	
    	this.props.adLiveStore.addImageChnList();
    /*	
        const name = event.target.name;
        if( event.target.files[0] ) { 
        	const file = event.target.files[0];
        	this.props.adLiveStore.addImageChnList(name, file, langCd);        	
        }
        */
    };
    ...
    handleChangeNewAdActivated = (event) => {
        ...
        this.props.adLiveStore.changeNewAdActivated(name, activated);
    }
   
    handleInsertMovie = () => {
    	this.props.adLiveStore.addNewMovie();
    }
    ...
    render() {
        ...
        const { chipData,isLoading, advertisements, paging, imageChnList, newShowCtg, newAd, newImageChn ,newImageEng ,newImageJpn ,addAdResult } = this.props.adLiveStore;
        const {isOpenFileDialog, isUploadable, isUploading, channelUserId, fileList, upload, uploadFile} = this.props.channelFileStore;

        const {value, setValue} = this.props.adLiveStore;
        ...
        return (
            ...
            <Container component="main" className={classes.mainContainer}>
                <div className={classes.appBarSpacer} />
			        ...
			        >
			          <MenuItem value={10}>客厅清单 (생방메인)</MenuItem>
			          <MenuItem value={11}>客厅主题  (생방리스트테마)</MenuItem>
			          <MenuItem value={12}>生活世界 (생방상세상단)</MenuItem>
			          <MenuItem value={13}>中央生活细节 (생방상세중앙)</MenuItem>
			          <MenuItem value={14}>生动细节  ( 생방상세하단)</MenuItem>
			          <MenuItem value={15}>详细的突破 (생방상세돌발)</MenuItem>
			        </Select>
			      </FormControl>
                ...
                <div className={classes.mainContent}>
                    ...
                    <a href='http://118.67.170.98:8000/'  target="_blank" className={classes.link} >
                    	<img src={'/images/ad'+newShowCtg.show_location+".png"} height="500" width="250" alt="show_location"/>
                    </a>
                   	</Typography>
...
export default withSnackbar(withRouter(withStyles(styles) (AdLiveList)));